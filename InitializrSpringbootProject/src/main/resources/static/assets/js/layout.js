...
document.addEventListener('DOMContentLoaded', function() {
    initializeLayout();
    // Si es layout.html, aplicar configuración por defecto si no existe
    if (document.getElementById('contentWrapper')) {
        if (!window.__staticLayoutConfig) {
            setupStaticLayout({
                defaultContent: '/views/layout/dashboard-content.html',
                defaultTitle: 'Panel Principal',
                defaultBreadcrumb: 'Inicio'
            });
        } else {
            applyStaticLayoutFromQuery();
        }
    }
});
...
function initializeLayout() {
    ...
    // Si estamos en layout.html estático, intentar configurar desde query params
    if (document.getElementById('contentWrapper')) {
        applyStaticLayoutFromQuery();
    }
}
...
/**
 * ================================================
 * STATIC LAYOUT SUPPORT (layout.html)
 * ================================================
 */

/**
 * Inicializa el layout estático con valores por defecto y query params
 * @param {{defaultContent?: string, defaultTitle?: string, defaultBreadcrumb?: string}} opts
 */
function setupStaticLayout(opts = {}) {
    const defaults = {
        defaultContent: '/views/layout/dashboard-content.html',
        defaultTitle: 'Panel Principal',
        defaultBreadcrumb: 'Inicio'
    ...
    // Guardar config en window para usarla si no hay query params
    window.__staticLayoutConfig = config;
    applyStaticLayoutFromQuery();
}

/**
 * Lee los parámetros de la URL y configura título, breadcrumb y contenido
 */
function applyStaticLayoutFromQuery() {
    try {
        const params = new URLSearchParams(window.location.search);
        const content = params.get('content') || (window.__staticLayoutConfig && window.__staticLayoutConfig.defaultContent);
        const title = params.get('title') || (window.__staticLayoutConfig && window.__staticLayoutConfig.defaultTitle) || document.title;
        const breadcrumb = params.get('breadcrumb') || (window.__staticLayoutConfig && window.__staticLayoutConfig.defaultBreadcrumb) || 'Inicio';

        // Título de la página y navbar
        document.title = title;
        const titleEl = document.getElementById('pageTitle');
        if (titleEl) titleEl.textContent = title;

        // Breadcrumb
        const bc = document.getElementById('breadcrumbList');
        if (bc) {
            // Limpiar excepto el primer ítem (Inicio)
            while (bc.children.length > 1) bc.removeChild(bc.lastElementChild);
            if (breadcrumb && breadcrumb !== 'Inicio') {
                ...
                .catch(err => {
                    ...
                    wrapper.innerHTML = `
                        <div class="alert alert-danger">
                            ...
                            Error cargando el contenido: <code>${content}</code>
                        </div>`;
                });
        }
    } catch (e) {
        console.error('Error aplicando layout estático', e);
    }
}
